...
echo "ctas,threads,scope,iters,size_bytes,bw_GBps" > "$CSV"
...
do
    ...
    do
        ...
        do
            TAG="c1_t${i}_s${j}_n${k}"
            RAW="$RAW_DIR/${TAG}.txt"

            echo "===== Running: ctas=1 threads=$i scope=$j iters=$k ====="
            ...
            mpirun \
                ...
                -np 2 -host r4,r6 \
                ./shmem_put_bw -c 1 -t "$i" -n "$k" -s "$j" -e 536870912\
                > "$RAW" 2>&1

            # 提取最终的测试数据表：从 "size(B)" 表头之后的纯数据行
            # 数据行格式示例: "4           None      0.001691"
            awk -v ctas=1 -v threads="$i" -v scope="$j" -v iters="$k" '
                /^size\(B\)/ { in_table=1; next }
                in_table {
                    # 跳过空行和非数据行
                    if ($0 ~ /^[[:space:]]*$/) next
                    if ($1 ~ /^[0-9]+$/ && NF >= 3) {
                        printf "%s,%s,%s,%s,%s,%s\n", ctas, threads, scope, iters, $1, $NF
                    }
                }
            ' "$RAW" | tee -a "$CSV"
        done
