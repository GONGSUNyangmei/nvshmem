...
echo "num_rc_per_pe,threads,scope,size_bytes,bw_GBps" > "$CSV"
...
do
    ...
    do
        ...
        echo "===== Running: num_rc_per_pe=$i threads=32 scope=$j ====="
        ...
        mpirun \
            ...
            -np 2 -host r4,r6 \
            ./shmem_put_bw -c 1024 -t 32 -n 1000 -s "$j" -b 512 -e 67108864\
            > "$RAW" 2>&1

        # 提取最终的测试数据表：从 "size(B)" 表头之后的纯数据行
        # 数据行格式示例: "4           None      0.001691"
        awk -v num_rc="$i" -v threads=32 -v scope="$j" '
            /^size\(B\)/ { in_table=1; next }
            in_table {
                # 跳过空行和非数据行
                if ($0 ~ /^[[:space:]]*$/) next
                if ($1 ~ /^[0-9]+$/ && NF >= 3) {
                    printf "%s,%s,%s,%s,%s\n", num_rc, threads, scope, $1, $NF
                }
            }
        ' "$RAW" | tee -a "$CSV"
    done
